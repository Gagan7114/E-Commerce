    ...
    <PlatformContext.Provider value={config}>
      <div className="app-layout" style={{ '--platform-color': config.color }}>
        <aside className="sidebar plat-sidebar">
          <div className="sidebar-brand" onClick={() => navigate(`/platform/${slug}`)} style={{ cursor: 'pointer' }}>
            {config.logo ? (
              <img className="brand-logo-img" src={config.logo} alt={config.name} onError={(e) => { e.target.style.display='none'; e.target.nextSibling.style.display='flex' }} />
            ) : null}
            <div className="brand-logo" style={{ background: config.color, display: config.logo ? 'none' : 'flex' }}>{config.icon}</div>
