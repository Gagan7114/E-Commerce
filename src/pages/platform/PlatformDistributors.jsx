...
export default function PlatformDistributors() {
  ...
  useEffect(() => {
    ...
    sapAPI.getDistributorOrders(selected, { page: 0, page_size: 50 })
      .then((r) => setOrders(r))
      .catch(() => {})
  ...
  useEffect(() => {
    ...
    sapAPI.getDistributorInvoices(selected, { page: 0, page_size: 50 })
  ...
  return (
    <>
      <div className="plat-page-header">
        <h1>Distributors</h1>
        <p>{total.toLocaleString()} distributors linked to {config.name} from SAP B1</p>
      </div>
        ...
        <div className="dist-layout">
          ...
          <div className="dist-list-panel">
            ...
            <div className="dist-list-body">
              {loading ? (
                <div className="plat-empty"><div className="loader" /> Loading distributors...</div>
              ) : distributors.length === 0 ? (
                <div className="plat-empty">No distributors found for {config.name}</div>
              ) : (
                distributors.map((d) => (
                  <div
                    key={d.CardCode}
                    ...
                    style={selected === d.CardCode ? { borderLeftColor: config.color } : undefined}
                  >
                    <div className="dist-item-avatar" style={{ background: d.Active === 'Y' ? config.color : '#b2bec3' }}>
                      {(d.CardName || '?')[0]}
                    </div>
                    ...
                    <div className="dist-item-balance">
                      {d.Balance != null && (
                        <span className={d.Balance < 0 ? 'negative' : 'positive'}>
            ...
            {totalPages > 1 && (
              <div className="pagination" style={{ padding: '10px 16px' }}>
                <button className="pg-btn" disabled={page === 0} onClick={() => setPage(0)}>&laquo;</button>
                <button className="pg-btn" disabled={page === 0} onClick={() => setPage(page - 1)}>&lsaquo;</button>
                <span className="pg-info">{page + 1} / {totalPages}</span>
                <button className="pg-btn" disabled={page >= totalPages - 1} onClick={() => setPage(page + 1)}>&rsaquo;</button>
                <button className="pg-btn" disabled={page >= totalPages - 1} onClick={() => setPage(totalPages - 1)}>&raquo;</button>
              </div>
            )}
          </div>
          ...
          <div className="dist-detail-panel">
            {!selected ? (
              ...
              <div className="plat-empty"><div className="loader" /> Loading details...</div>
            ) : !detail ? (
              <div className="plat-empty">Failed to load details</div>
            ) : (
              <>
                <div className="dist-detail-header">
                  <div className="dist-detail-avatar" style={{ background: detail.distributor.Active === 'Y' ? config.color : '#b2bec3' }}>
                    {(detail.distributor.CardName || '?')[0]}
                  </div>
                  <div>
                    <h2 className="dist-detail-name">{detail.distributor.CardName}</h2>
                    <div className="dist-detail-code">
                      {detail.distributor.CardCode}
                      {detail.distributor.Chain && <span> &middot; {detail.distributor.Chain}</span>}
                      <span> &middot; {detail.distributor.CardType === 'C' ? 'Customer' : 'Vendor'}</span>
                    </div>
                  </div>
                  <div className="dist-detail-status">
                    <span className={`plat-status ${detail.distributor.Active === 'Y' ? 'dispatched' : 'loading'}`}>
                      {detail.distributor.Active === 'Y' ? 'Active' : 'Inactive'}
                    </span>
                  </div>
                </div>
                ...
                <div className="dist-tab-content">
                  {detailTab === 'info' && <InfoTab data={detail.distributor} />}
                  {detailTab === 'addresses' && <AddressesTab addresses={detail.addresses} />}
...
function InfoTab({ data }) {
  const fields = [
    ...
    ['Balance', data.Balance != null ? `${data.Currency || 'INR'} ${Number(data.Balance).toLocaleString()}` : null],
    ['Credit Line', data.CreditLine != null ? `${data.Currency || 'INR'} ${Number(data.CreditLine).toLocaleString()}` : null],
...
function AddressesTab({ addresses }) {
  if (!addresses || addresses.length === 0) return <div className="plat-empty">No addresses found</div>
  return (
    <div className="dist-cards-grid">
      {addresses.map((a, i) => (
        <div key={i} className="dist-address-card">
          <div className="dist-address-type">{a.AdresType === 'S' ? 'Ship To' : a.AdresType === 'B' ? 'Bill To' : a.AdresType}</div>
          <div className="dist-address-label">{a.Address}</div>
          <div className="dist-address-detail">{[a.Street, a.Block, a.City, a.State, a.ZipCode, a.Country].filter(Boolean).join(', ')}</div>
        </div>
      ))}
    </div>
  )
...
function ContactsTab({ contacts }) {
  if (!contacts || contacts.length === 0) return <div className="plat-empty">No contacts found</div>
  return (
    <div className="dist-cards-grid">
      {contacts.map((c, i) => (
        <div key={i} className="dist-address-card">
          <div className="dist-address-type">{c.Position || 'Contact'}</div>
          <div className="dist-address-label">{[c.FirstName, c.LastName].filter(Boolean).join(' ') || c.Name}</div>
          <div className="dist-address-detail">
            {c.Tel1 && <span>Phone: {c.Tel1} </span>}
            {c.Email && <span>Email: {c.Email}</span>}
          </div>
          <span className={`plat-status ${c.Active === 'Y' ? 'dispatched' : 'loading'}`} style={{ marginTop: '6px', display: 'inline-block' }}>
...
function DocsTab({ data, loading, type }) {
  if (loading) return <div className="plat-empty"><div className="loader" /> Loading {type}s...</div>
  if (!data.data || data.data.length === 0) return <div className="plat-empty">No {type.toLowerCase()}s found</div>
  return (
    <>
      <div style={{ fontSize: '12px', color: '#95a5a6', marginBottom: '10px' }}>{data.count} {type.toLowerCase()}(s)</div>
      <div className="table-wrapper">
        <table className="data-table">
          <thead>
            <tr><th>Doc #</th><th>Date</th><th>Due Date</th><th>Total</th><th>Currency</th><th>Status</th><th>Vendor Ref</th></tr>
          </thead>
          <tbody>
            {data.data.map((doc) => (
              <tr key={doc.DocEntry}>
                ...
                <td>{doc.DocCur}</td>
                <td><span className={`plat-status ${doc.DocStatus === 'O' ? 'loading' : 'dispatched'}`}>{doc.DocStatus === 'O' ? 'Open' : 'Closed'}</span></td>
                <td>{doc.VendorRef || '-'}</td>
              </tr>
            ))}
          </tbody>
