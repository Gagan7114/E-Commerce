    ...
    <div className="app-layout">
      ...
      <aside className="sidebar dist-sidebar">
        <div className="sidebar-brand" onClick={() => navigate('/dashboard')} style={{ cursor: 'pointer' }}>
          <div className="brand-logo" style={{ background: '#6c5ce7' }}>S</div>
        ...
        <nav className="sidebar-nav">
          ...
          <button className="plat-nav-item plat-back-btn" onClick={() => navigate('/dashboard')}>
            &larr; Main Dashboard
          </button>
        </nav>
              ...
              <div className="dist-list-body">
                {loading ? (
                  ...
                  distributors.map((d) => (
                    ...
                    >
                      <div className="dist-item-avatar" style={{ background: d.Active === 'Y' ? '#00b894' : '#b2bec3' }}>
                        {(d.CardName || '?')[0]}
                      </div>
                      ...
                      <div className="dist-item-balance">
                        {d.Balance != null && (
                          <span className={d.Balance < 0 ? 'negative' : 'positive'}>
                            {d.Currency || 'INR'} {Number(d.Balance).toLocaleString()}
                          </span>
                        )}
                      </div>
                    </div>
              ...
              {!selected ? (
                ...
                <>
                  ...
                  <div className="dist-detail-header">
                    <div className="dist-detail-avatar" style={{ background: detail.distributor.Active === 'Y' ? '#00b894' : '#b2bec3' }}>
                      {(detail.distributor.CardName || '?')[0]}
                    </div>
                    <div>
                      <h2 className="dist-detail-name">{detail.distributor.CardName}</h2>
                      <div className="dist-detail-code">{detail.distributor.CardCode}</div>
                    </div>
      ...
      {addresses.map((a, i) => (
        <div key={i} className="dist-address-card">
          <div className="dist-address-type">
            {a.AdresType === 'S' ? 'Ship To' : a.AdresType === 'B' ? 'Bill To' : a.AdresType}
          </div>
          <div className="dist-address-label">{a.Address}</div>
          <div className="dist-address-detail">
            {[a.Street, a.Block, a.City, a.State, a.ZipCode, a.Country].filter(Boolean).join(', ')}
          </div>
        </div>
      ))}
    </div>
...
function DocsTab({ data, loading, type }) {
  if (loading) {
    return <div className="plat-empty"><div className="loader" /> Loading {type}s...</div>
  }
          ...
          <tbody>
            {data.data.map((doc) => (
              <tr key={doc.DocEntry}>
                ...
                <td>{doc.DocCur}</td>
                <td>
                  <span className={`plat-status ${doc.DocStatus === 'O' ? 'loading' : 'dispatched'}`}>
