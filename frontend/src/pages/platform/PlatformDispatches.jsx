          ...
          {total > 0 && (
            <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '12px 16px 0' }}>
              <button
                ...
                style={{ fontSize: '11px', padding: '4px 12px', color: '#e74c3c', borderColor: 'rgba(231,76,60,0.3)' }}
                    ...
                    {dispatches.map((d, i) => (
                      <>
                        ...
                        >
                          ...
                          <td>{d.dispatch_date || d.created_at?.split('T')[0] || '-'}</td>
                          <td>
                            <span className={`plat-status ${d.mode === 'quick' ? 'loading' : 'dispatched'}`}>
                        ...
                        {expanded === d.id && (
                          <tr key={`${d.id}-details`}>
                            <td colSpan={11} style={{ background: '#f8f9fc', padding: '14px 20px' }}>
                              <div style={{ fontSize: '12px', color: '#636e72' }}>
                                ...
                                <strong> Created:</strong> {d.created_at ? new Date(d.created_at).toLocaleString() : '-'}
                              </div>
                                ...
                                <div style={{ marginTop: '10px' }}>
                                  ...
                                  {d.po_details.map((po, j) => (
                                    <div key={j} style={{ fontSize: '12px', padding: '4px 0', color: '#636e72' }}>
                                      {po.po_name} — {po.sku_code && `SKU: ${po.sku_code} | `}{po.quantity != null ? `Qty: ${po.quantity} | ` : ''}{po.weight_kg} kg
                                    </div>
                                  ))}
                                </div>
                              )}
